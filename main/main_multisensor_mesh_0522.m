...
for idx_link = 1:size_joint % 遍历所有连杆
    ...
    parfor idx_point = 1:size_X
        ...
        pstart = idx_point;
        % options.start_points = pstart;
        % clear options;
        % options.end_points = [];

        % options.W = ones(size_X,1);
...
for idx_config = 1:size_sim
    ...
    for idx_link = 2:size_joint-1 % 遍历所有连杆 (末端连杆对应空矩阵)
        ...
        pstart = edge_candidate_all1{1,idx_link}(1,1); % 起点序号
        phi = getGeodesicDistanceHeat(X,F,pstart);
        ...
        pend = edge_candidate_all1{1,idx_link}(1,2); % 终点序号
        %
